...
const Dashboard = () => {
  ...
  useEffect(() => {
    const processedData = caseData
      .map((item) => {
        ...
        if (isNaN(filingDate)) {
          console.warn(
            `Invalid filing_date for item ID ${item.id}:`,
            item.filing_date
          );
        ...
        if (item.decision_date && isNaN(decisionDate)) {
          console.warn(
            `Invalid decision_date for item ID ${item.id}:`,
            item.decision_date
          );
          return null; // Exclude this item or handle it as needed
        }
        ...
        return {
          ...
          disposalDuration: decisionDate
            ? (decisionDate - filingDate) / (1000 * 60 * 60 * 24)
            : null,
        };
      })
  ...
  const disposedCases = data.filter(
    (item) => item.Nature_of_Disposal !== ""
  ).length;
        ...
        <Grid item xs={12}>
          <ChartWrapper title="Hearing Count vs Disposal Time">
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <YAxis
                type="number"
                dataKey="disposalTime"
                name="Disposal Time (days)"
              />
              <Tooltip cursor={{ strokeDasharray: "3 3" }} />
              <Scatter
                name="Cases"
                data={hearingVsDisposalTime}
                fill="#8884d8"
              />
            </ScatterChart>
          </ChartWrapper>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper elevation={3} sx={{ p: 2, textAlign: "center" }}>
            ...
            <Typography variant="h4">
              {(
                data.reduce((sum, item) => sum + item.hearing_count, 0) /
                data.length
              ).toFixed(2)}
            </Typography>
          </Paper>
